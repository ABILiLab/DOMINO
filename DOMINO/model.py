...
class EdgePerturbation_GraphCL(Module):
    def __init__(self, input_dim, hidden_dim, output_dim, proj_dim, graph_neigh, graph_edge, dropout=0.5, act=F.relu):
        super(EdgePerturbation_GraphCL, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.proj_dim = proj_dim
    ...
    def forward(self, feat, feat_a, adj, edge_adj):
        # Original view
        h = self.encode(feat, adj)
        # Diffusion-augmented view
        h_g = self.encode(feat, edge_adj) 
        # Feature-shuffled negative sample
        perm = torch.randperm(feat.size(0), device=feat.device)
        shuf_h = self.encode(feat[perm], adj)
        # Reconstructed feature
        ...
        # Diffusion-augmented view graph representation
        g_g = self._sparse_avg(h_g, self.graph_edge)
        g_g = self.sigm(g_g)  
        g_g = self.mlp(g_g)
...
class FeatureMask_GraphCL(Module):
    def __init__(self, input_dim, hidden_dim, output_dim, proj_dim, graph_neigh, dropout=0.5, act=F.relu):
        super(FeatureMask_GraphCL, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.proj_dim = proj_dim
    ...
    def forward(self, feat, feat_mask, adj):
        # Original view
        h = self.encode(feat, adj)
        # Diffusion-augmented view
        h_g = self.encode(feat_mask, adj) 
        # Feature-shuffled negative sample
        perm = torch.randperm(feat.size(0), device=feat.device)
        shuf_h = self.encode(feat[perm], adj)
        # Reconstructed feature
        ...
        # Diffusion-augmented view graph representation
        g_g = self._sparse_avg(h_g, self.graph_neigh)
        g_g = self.sigm(g_g)  
        g_g = self.mlp(g_g)
...
class Single_view_GraphCL(Module):
    def __init__(self, input_dim, hidden_dim, output_dim, proj_dim, graph_neigh, dropout=0.5, act=F.relu):
        super(Single_view_GraphCL, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.proj_dim = proj_dim
    ...
    def forward(self, feat, feat_a, adj):
        ...
        ret = self.disc(g, z, shuf_z)  
        
        return emb, ret


class Recon_only_model(Module):
    def __init__(self, input_dim, hidden_dim, output_dim, dropout=0.5, act=F.relu):
        super(Recon_only_model, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        self.dropout = dropout
